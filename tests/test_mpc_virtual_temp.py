...
def test_virtual_temp_is_capped_near_sensor():
    key = "t:climate.y:t20.0"
    mpc._MPC_STATES.pop(key, None)
    ...
    with patch(
        ...
        out1 = mpc.compute_mpc(inp, params)
        assert out1 is not None
        assert state.virtual_temp == pytest.approx(20.0, rel=0, abs=1e-6)

    # Large time delta would push virtual_temp far above sensor without cap.
    with patch(
        "custom_components.better_thermostat.utils.calibration.mpc.monotonic",
        side_effect=[3100.0],
    ):
        out2 = mpc.compute_mpc(inp, params)
        assert out2 is not None
        assert state.virtual_temp == pytest.approx(
            20.0 + mpc.VIRTUAL_TEMP_MAX_DEVIATION_K, rel=0, abs=1e-6
        )
